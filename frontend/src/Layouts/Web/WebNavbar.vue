...
const dropdownMenu = [
	{ id: 1, name: 'Account Settings', path: '/profile' },
	{ id: 2, name: 'Your Course', path: '/course' },
	{ id: 3, name: 'Sign In', path: '/login' },
]
		...
		<el-header class="flex justify-between items-center border border-bottom-1">
			...
			<div v-if="profile" class="flex gap-4 items-center">
						...
						<div class="space-y-4" v-for="notification in notificationlist" :key="notification.id">
							<div class="flex items-start space-x-3 mb-3">
								<img class="w-8 h-8 rounded-full"
									src="https://i.pinimg.com/564x/3d/de/f0/3ddef0fc2c011f22b616afe111addcdf.jpg"
									alt="user-avatar" />
								<div class="flex-1">
									<p class="text-sm"><span class="font-semibold"></span> {{ notification.description
										}}</p>
								</div>
								<span class="text-xs text-muted-foreground dark:text-muted">{{ notification.created_at
									}}</span>
							</div>
						</div>
					</div>
				...
				<Menu as="div" class="relative inline-block text-left z-50">
					<div>
						<MenuButton>
							<user-profile :Src="profile == null ? './src/assets/avatar/avatar-profile.jpg' : profile" />
						</MenuButton>
					</div>
					...
					<transition enter-active-class="transition ease-out duration-100"
						...
						<MenuItems
							...
							<div class="py-1">
								...
								<MenuItem>
								<span @click="confirmLogoutForm"
									class="text-gray-700 block px-4 py-2 text-sm cursor-pointer hover:bg-gray-200">Sign out</span>
								</MenuItem>
							</div>
						</MenuItems>
					</transition>
