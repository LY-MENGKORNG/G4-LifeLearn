<template>
  <SystemLayout>
    <div class="flex items-center space-x-20 border-y-2 py-3 border-gray-500">
      <router-link to="/system/material">
        <h6>Stream</h6>
      </router-link>
      <router-link to="/system/people">
        <h6>People</h6>
      </router-link>
      <router-link to="/system/missing">
        <h6>Missing</h6>
      </router-link>
      <router-link to="/system/done">
        <h6>Done</h6>
      </router-link>
    </div>
    <div class="teacher flex-cols p-8 bg-white space-y-14 mx-40 mt-10 scrollable-content">
      <div class="teacher flex-cols space-y-3 ">
        <h1 class="text-blue-500 text-4xl border-b-2 border-blue-500">Teachers</h1>
        <div class="flex justify-between space-x-8 items-center">
          <div class="flex items-center space-x-8 py-1">
            <img class="top-12 left-48 w-10 h-10 object-cover rounded-full shadow-md"
              src="data:image/jpeg;base64,"
              alt="Course Image" />
            <p>Lavy hou</p>
          </div>
          <el-icon :size="26">
            <Message />
          </el-icon>
        </div>
      </div>
      <div class="stuent space-y-3">
        <div class="border-b-2 border-blue-500 flex justify-between">
          <h1 class="text-blue-500 text-4xl space-y-3">Classmate</h1>
          <p>25 students</p>
        </div>
        <StudentList class="flex flex-cols " v-for="index in 8" :key="index" />

      </div>
    </div>
  </SystemLayout>
</template>

<script setup lang="ts">

import { RouterLink } from 'vue-router'
import SystemLayout from '@/Layouts/System/SystemLayout.vue'
import StudentList from '@/Components/Classroom/StudentList.vue'
import { Message } from '@element-plus/icons-vue'
</script>


<style scoped>
.scrollable-content {
  height: calc(100vh - 56px);
  overflow-y: auto;
}
</style>